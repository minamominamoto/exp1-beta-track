# Exp 1 β Pre-registration v1.4 — Frame Reversion in Instruction-Tuned LLMs

**Registered**: 2026-05-18 (OSF archived 2026-05-18 17:53 JST)
**Author**: みなもと みなも(Minamo Minamoto)
**ORCID**: 0009-0002-1201-5704
**OSF DOI**: https://doi.org/10.17605/OSF.IO/9EYXR
**OSF Registration**: https://osf.io/9eyxr/
**GitHub (development, private)**: https://github.com/minamominamoto/papers-under-review-/tree/3fdc049/exp1-beta-track
**GitHub (public mirror)**: https://github.com/minamominamoto/exp1-beta-track
**Companion files (frozen at registration, all SHA256 computed 2026-05-18)**:
...
- `claude--exp1-llm-judge-prompt.md` v1.0 — SHA256: `9b7fdefa5cd0b2f192e37cf4b8cfb078158d1a33610eb7b8b120bf9f34ff28f8`
- `claude--exp1-postprobe-recompute.py` v1.0 — SHA256: `6972ee1bb50803049a17200149c3acb7678f565e9ec73fb967505ddfa5b6dff8`
- `claude--exp1-blind-audit-coding-scheme.md` v1.0 — SHA256: `8876261e867541aed26a38378d8f4b6ca20c904d19b8a936b2db9f36e5a98810`

**Post-registration companion file (added in v1.2, see §13.8)**:
- `claude--exp1-llm-judge-driver.py` v1.0 — SHA256: `47a34754199c967779a7caee76fdc604f0026b0fa9e34672be40af8f32980401`

(The 6 SHAs above are confirmed values **frozen at OSF registration**. The 7th file is a post-registration implementation of §7 of the LLM-judge prompt spec; it does not alter any frozen design choice and is documented in §13.8.)

**v1 → v1.1 changes**: Fast-track execution. Original v1 planned 1-week cooling-off period (2026-05-18 → 2026-05-25 registration → 2026-05-26 run). v1.1 collapses this to same-day registration + execution after author judgment that MACIR I/II/III cross-checks had sufficiently stabilized the design. **Submit-before-execution order is preserved**: OSF submission and GitHub commit precede the first pilot run, in compliance with pre-registration semantics.

**v1.1 → v1.2 changes (post-registration, non-design)**: This version embeds the OSF DOI (`10.17605/OSF.IO/9EYXR`) and the registration GitHub commit hash (`3fdc049`) into the header and §12, and adds §13.8 documenting the Stage A2 update and the LLM-judge driver implementation. **No design choices in §1–§12 are altered.** The frozen-at-registration v1.1 document (SHA256 `e2722b07...`) is preserved unmodified at the OSF archive and in both GitHub repositories.

**v1.2 → v1.3 changes (Step C complete, non-design)**: This version fills §13.6 with the C1a pilot results (executed 2026-05-18 18:46 JST, Modal A100, N=10×3=30 generations, seed=42) and records the adopted C1a variant per §5.3 selection rule. **No design choices in §1–§12 are altered.** The Modal output `stage1_pilot_20260518_094625.json` and log `logs/stage1_20260518_094625.log` are SHA-fixed at this v1.3 commit; their SHA256s are recorded in §13.6.

**v1.3 → v1.4 changes (Steps D + E complete, with one model-substitution deviation)**: This version records the Stage 1 main run (Step D, executed 2026-05-18 19:03 JST, Modal A100, N=900 generations, seeds=[42,7,123]) and the Layer 2 LLM-judge classification (Step E, executed 2026-05-18 20:24–20:40 JST, Anthropic Message Batches API, 3600 judgments, claude-sonnet-4-6). §13.10 contains complete Layer 1 + Layer 2 aggregates, Cohen's κ cross-layer agreement, and §6.4 GO/NO-GO decision. §13.9 documents one deviation from §1: the originally registered judge model `claude-3-5-sonnet-20241022` was retired by Anthropic on 2026-01-05; it was substituted with `claude-sonnet-4-6` (same Sonnet family, active status). All other §1–§12 design choices are preserved. The substitution is also reported as a row in the §15 anticipated-deviations table.
...
- **Version**: v1.4 (Steps D+E complete; v1.1 archived at OSF as frozen)
- **Registration date**: 2026-05-18 (fast-track, same-day as Stage 1 execution)
- **Stage 1 run window**: 2026-05-18 onwards (immediately after OSF submission)
- **Stage 1.5 run window**: 2026-05-19 onwards (conditional on §6.4 GO)
...
### 5.3 Pilot (2026-05-18, after OSF submission)
...
### 5.4 Main run (2026-05-18, immediately after pilot)
...
| Item | Value |
|---|---|
| Pre-registration version | v1.4 (current; v1.1 archived at OSF) |
| Pre-registration commit hash (v1.1, recorded at OSF registration) | `3fdc049` (papers-under-review-/exp1-beta-track/, 2026-05-18) |
| Pre-registration commit hash (v1.2, Stage A2) | `38e4e08` (papers-under-review-) / `acb81e7` (exp1-beta-track) |
| Pre-registration commit hash (v1.3, Step C) | `ce7a42c` (papers-under-review-) / `475d564` (exp1-beta-track) |
| Pre-registration commit hash (v1.4, Steps D+E) | (recorded at the v1.4 GitHub commit) |
| Stage 1 main output (Modal) | `stage1_main_20260518_100316.json` SHA256 `7f726e41361d650ab3a795d9d6c4e2b5fc101788ba5b0d09a0850ba99f1d966c` |
| Stage 1 main log (Modal) | `logs/stage1_20260518_100316.log` SHA256 `dd56799312e292a5948c5e37a5e5eb96b7cd4e0e9353d749412fa3f8049ad5d9` |
| LLM-judge output (Layer 2) | `judge_results_20260518_100316.json` SHA256 `0273ac331d02f35f875d2a8f31826d8e9ffea4b1ce6f735be269fd477b07972d` |
| LLM-judge driver v1.2 (active) | SHA256 `3d07e6d0a30a145cd917a9c043dd9275dffa7f205149a7f3b1866f54fb7bee01` (model substitution per §13.9) |
| LLM-judge driver v1.1 (superseded) | SHA256 `8256bb41c371dd5c1709e24466c86db2cd914de906c4ce94124dd17732671c85` (composite-key fix; superseded by v1.2 after model retirement) |
| OSF DOI | `10.17605/OSF.IO/9EYXR` (resolved at https://doi.org/10.17605/OSF.IO/9EYXR) |
| OSF Registration page | https://osf.io/9eyxr/ |
| OSF archive date | 2026-05-18 17:53 JST |
| Public GitHub mirror | https://github.com/minamominamoto/exp1-beta-track |
| v1.1 SHA256 (frozen at OSF) | `e2722b07ac0fe9f417bc3fb811629fc1b955939c838c3e457791541dce922b86` |
| protocol v4.1 SHA256 | `95c4bc5f94059ac7419efff912db9971ed1351183326c9b9b44ce6459f5aae78` |
| prompts JSON v1.0 SHA256 | `f2a4abbf51021e6cfa2e8b501c33166af5fb097bcda020afc0c4353d3ecd7ff5` |
| stage1-main.py v1.0 SHA256 | `d80902fef426b1138ce2b9614f3ab4494807388dc9103c9bd5dc2477997199a0` |
| llm-judge-prompt.md v1.0 SHA256 | `9b7fdefa5cd0b2f192e37cf4b8cfb078158d1a33610eb7b8b120bf9f34ff28f8` |
| postprobe-recompute.py v1.0 SHA256 | `6972ee1bb50803049a17200149c3acb7678f565e9ec73fb967505ddfa5b6dff8` |
| blind-audit-coding-scheme.md v1.0 SHA256 | `8876261e867541aed26a38378d8f4b6ca20c904d19b8a936b2db9f36e5a98810` |
| llm-judge-driver.py v1.0 SHA256 (post-registration, §13.8) | `47a34754199c967779a7caee76fdc604f0026b0fa9e34672be40af8f32980401` |
| Modal image SHA | [recorded at first Stage 1 build, before main run] |

If any of the 6 companion files frozen at OSF registration is modified after registration, the modification is a deviation and reported in the manuscript. The SHA of this pre-registration document (v1.1) is fixed by its OSF archive and GitHub commit `3fdc049`; the v1.2 document is a post-registration update that does not modify §1–§12 design content (it only embeds the DOI and the post-registration LLM-judge driver as described in §13.8).
...
### 13.6 Stage 1 Pilot results (executed 2026-05-18 18:46 JST)

Pilot run executed on Modal A100-40GB to select the C1a variant per §5.3 selection rule.

**Operational parameters**:
- Modal app: `exp1-stage1-main`, GPU `A100-40GB`, image based on `python:3.11`
- Model: `Qwen/Qwen2.5-7B-Instruct` (fp16, downloaded via HuggingFace at run time)
- Prompts: 10 from `health` bucket (the first 10 of `claude--exp1-prompts-stage1.json`)
- Conditions: C1a-i / C1a-ii / C1a-iii (each with the same 10 health prompts)
- Seeds: `[42]` (one seed for pilot, consistent with §5.3 pilot spec)
- Total generations: 3 conditions × 10 prompts × 1 seed = **30 generations**
- Decoding: `max_new_tokens=150`, `temperature=0.7`, `top_p=0.9`, ChatML formatting
- Wall clock: pilot run completed in approximately 4 minutes (including model download)

**Results** (Family A primary, B/C/D secondary; rates computed by regex Layer 1 per §7):

| Condition | Family A (qualified-response) | Family B (directness) | Family C (unqualified) | Family D (refusal) |
|---|---|---|---|---|
| C1a-i | r = 0.30 | 0.30 | 0.00 | 0.00 |
| C1a-ii | r = 0.30 | 0.10 | 0.00 | 0.00 |
| **C1a-iii** | **r = 0.10** | 0.10 | 0.00 | 0.00 |

**Adopted variant for main run: C1a-iii** (selected by §5.3 rule: lowest Family A rate).

Rationale: C1a-iii achieves r(Family A) = 0.10, the lowest of the three, indicating the strongest distribution-shift effect (most aggressive suppression of qualified-response markers). C1a-i and C1a-ii both yield r = 0.30 — tied on the primary criterion, but C1a-i would have won the tie-break per §5.3 step 2 (higher Family B). Since C1a-iii dominates on the primary criterion, no tie-break is invoked.

**Modal output files** (cached on `exp1-cache` volume):
- `stage1_pilot_20260518_094625.json` (pilot results JSON; SHA256: `226d5d72d47ec51d82baff3a5cdfc4e6bd619a1988e61727b2c30b5a8e1a3355`)
- `logs/stage1_20260518_094625.log` (run log; SHA256: `093db86bdb64d0de00a2abc3a0c34114d74fca237a8a00f75acb5c4dc2d31914`)
- Local copies: `cache/results/pilot_results_20260518_094625.json` and `cache/results/pilot_log_20260518_094625.log`

**Note on prompt overlap with main run**:
The 10 health prompts used in the pilot are a subset of the 25 health prompts used in the main run. Pilot generations themselves are **not pooled** into the main analysis; the main run will re-generate responses with seeds `[42, 7, 123]` (3 seeds, not 1), and only the main run's 900 generations enter the confirmatory analysis. Pilot data informs only the C1a variant selection step. The selected variant (C1a-iii) is fixed for the entire confirmatory analysis from this commit forward.
...
### 13.8 Stage A2 update — DOI embed and LLM-judge driver implementation (post-registration, 2026-05-18)

This subsection documents the v1.1 → v1.2 update applied **after** OSF registration was archived. **No design choices in §1–§12 are altered.** The update is mechanical: it embeds the DOI/commit-hash values that were unknown at the registration moment, and it records the implementation of the LLM-judge driver previewed in §7 of `claude--exp1-llm-judge-prompt.md`.

**Timeline**:
- 2026-05-18 17:53 JST: OSF registration `9eyxr` archived; DOI `10.17605/OSF.IO/9EYXR` assigned
- 2026-05-18 evening JST: v1.2 update committed to both `papers-under-review-/exp1-beta-track/` (private development repo) and `exp1-beta-track/` (public mirror at https://github.com/minamominamoto/exp1-beta-track)
- 2026-05-18 evening JST: LLM-judge driver `claude--exp1-llm-judge-driver.py` v1.0 (SHA256 `47a34754199c967779a7caee76fdc604f0026b0fa9e34672be40af8f32980401`) authored as a separate file, per the spec in §7 of `claude--exp1-llm-judge-prompt.md` (frozen at registration, SHA256 `9b7fdefa...`)

**LLM-judge driver scope (frozen settings only, no new design choices)**:
- Implements verbatim the 4 family templates (§2.1–§2.4 of `claude--exp1-llm-judge-prompt.md`)
- Uses frozen model `claude-3-5-sonnet-20241022`, `temperature=0`, `max_tokens=10`
- Implements frozen parsing rule (§3.1) and frozen retry policy (§4.1)
- Adds Anthropic Message Batches API support (operational efficiency; equivalent semantics to the streaming/per-call mode also implemented as fallback)
- Adds resume logic via `(generation_id, family)` composite key (operational; idempotent re-run after interruption)
- Verifies SHA256 of `claude--exp1-llm-judge-prompt.md` against the registered value at startup; aborts (or warns with `--skip-sha-check`) on mismatch to detect post-hoc prompt drift

**Why this is not a deviation**: §7 of `claude--exp1-llm-judge-prompt.md` (frozen at registration) explicitly states "The driver script `claude--exp1-llm-judge-driver.py` (to be authored as a separate file) implements: [pseudo-code]." The v1.0 implementation conforms to this pseudo-code with operational enhancements (Batch API, resume, SHA verification) that do not alter the judge's input prompts, model, parameters, parsing, or retry semantics. The driver's SHA256 is fixed at this v1.2 update and any future modification would itself be a deviation reported in the manuscript.

### 13.9 Judge model substitution — deviation from §1 (2026-05-18)

**Deviation declared explicitly.** Pre-registration §1 frozen `JUDGE_MODEL = claude-3-5-sonnet-20241022`. At the time of Step E execution (2026-05-18), this model was no longer available on the Anthropic API (Anthropic announced deprecation 2025-08-13; retirement effective 2026-01-05, per Anthropic deprecation documentation accessed 2026-05-18).

Two attempted runs with the originally registered model returned `BetaNotFoundError(message='model: claude-3-5-sonnet-20241022', type='not_found_error')` for all 3600 batched requests, confirming the retirement.

**Substitution**: `claude-sonnet-4-6` (Anthropic Sonnet 4.6, released 2026-02-17, active status).

**Driver version transition**:
- v1.0 (`47a34754...`): original implementation, used retired model, contained composite-key bug (see v1.0 → v1.1 below)
- v1.1 (`8256bb41...`): fixed `_gen_id()` composite-key collision in batch submission (`{id}__{condition}__{seed}` instead of `{id}` alone); same retired model
- **v1.2 (`3d07e6d0...`): substituted JUDGE_MODEL to `claude-sonnet-4-6`** — active. This is the version actually used for the recorded judge_results.

All other §1 settings preserved: `temperature=0`, `max_tokens=10`, ChatML, Anthropic Message Batches API, separate `ANTHROPIC_API_KEY_JUDGE` env var. All §2 (4 family prompt templates), §3 (parsing rule), §4 (retry policy) unchanged.

**Rationale for choice of substitute model**:
- Same model family (Sonnet line) → minimizes capability discontinuity
- Active status (released 2026-02-17, no near-term retirement risk) → reproducible during analysis window
- Batch API compatible → preserves cost efficiency (~$3.65)
- Better calibration on instruction-following than Sonnet 3.5 → may improve judge agreement on the "one word only" instruction (observed: 0% malformed across 3600 calls, vs. expected ~1% in §3.3)

**Implications for Layer 2 results**:
The judge ratings recorded in §13.10 were generated by `claude-sonnet-4-6`, not by `claude-3-5-sonnet-20241022`. Any difference in judge calibration between these models is a confound for Layer 2 family rates. The Layer 1 regex outputs (§13.10) are unaffected by this substitution. Cross-layer agreement (Cohen's κ) is reported alongside the layer-specific rates so that readers can independently assess the contribution of the substitution.

**Reproducibility**:
This judge run cannot be exactly reproduced with the originally registered model. If Anthropic restores access to retired models (per their commitments on model preservation, https://www.anthropic.com/research/deprecation-commitments), future replication studies can attempt reproduction. Until then, replications should use `claude-sonnet-4-6` (or its successor in the Sonnet line, with substitution explicitly noted) and acknowledge model-calibration as a source of potential variation.

This deviation is also recorded in §15 (anticipated deviations table) under the new row "Judge model retired by Anthropic".

### 13.10 Stage 1 main run results (executed 2026-05-18 19:03 JST)

Stage 1 main run executed on Modal A100-40GB. The judge model substitution described in §13.9 was applied for Layer 2; Layer 1 regex was applied per §7 (frozen).

**Operational parameters** (unchanged from §3, §4, §5):
- Modal app: `exp1-stage1-main`, GPU `A100-40GB`
- Model: `Qwen/Qwen2.5-7B-Instruct` (fp16)
- Prompts: 100 (25 per bucket × 4 buckets: health, legal, finance, interpersonal)
- Conditions: C0 (neutral) + C1a-iii (selected per §13.6) + C1b (few-shot direct)
- Seeds: [42, 7, 123]
- Total generations: 100 × 3 × 3 = **900 generations**
- Wall clock: ~25 minutes (Stage 1 main); ~16 minutes (Layer 2 batch judge)

**Layer 1 (regex) results**:

| Condition | Family A (qualified) | Family B (directness) | Family C (unqualified) | Family D (refusal) | n |
|---|---:|---:|---:|---:|---:|
| C0 (neutral) | 0.557 | 0.097 | 0.090 | 0.000 | 300 |
| C1a-iii | 0.243 | 0.130 | 0.017 | 0.000 | 300 |
| C1b (few-shot) | 0.073 | 0.310 | 0.047 | 0.000 | 300 |

**Layer 2 (LLM-judge `claude-sonnet-4-6`) results**:

| Condition | Family A (qualified) | Family B (directness) | Family C (unqualified) | Family D (refusal) | n |
|---|---:|---:|---:|---:|---:|
| C0 (neutral) | 0.810 | 0.100 | 0.300 | 0.000 | 300 |
| C1a-iii | 0.650 | 0.293 | 0.420 | 0.003 | 300 |
| C1b (few-shot) | 0.573 | 0.740 | 0.453 | 0.000 | 300 |

**Layer 2 quality control**:
- 3600 judgments submitted; 0 malformed / 0 errored (100% one-word compliance)
- Batch ID: `msgbatch_018VUBKnGLqunNZY2Dv9bG9z`
- Batch latency: ~16 minutes (in_progress → ended)
- Judge SDK version: `anthropic==0.102.0`

**§6.4 GO/NO-GO decision** (Family A primary; Layer 2 is the confirmatory layer per §8.2):

| Layer | r(C0, A) | r(C1b, A) | delta_elicit | Threshold | Verdict |
|---|---:|---:|---:|---:|---|
| Layer 1 (regex, secondary) | 0.557 | 0.073 | **0.483** | 0.150 | GO |
| **Layer 2 (judge, confirmatory)** | 0.810 | 0.573 | **0.237** | 0.150 | **✅ GO** |

**Conclusion**: Stage 1 → Stage 1.5 **GO**. Stage 1.5 (within-conversation reversion test) proceeds per §6.

**§8.4 Cross-layer agreement on Family A primary outcome**:

| Metric | Value |
|---|---:|
| N | 900 |
| Layer 1 yes-rate | 0.291 |
| Layer 2 yes-rate | 0.678 |
| Raw agreement | 0.567 |
| **Cohen's κ** | **0.245** (poor agreement per §6.3 interpretation guidance) |

Confusion matrix (Layer 1 ↔ Layer 2 on Family A):
...
**Observations on cross-layer divergence**:
- 369 records (41% of N=900) are detected by Layer 2 (LLM-judge) but not Layer 1 (regex). These represent semantic markers of qualified-response that the regex set in §7 does not capture (e.g., paraphrased hedges, embedded uncertainty acknowledgments, indirect consult-recommendations). Five examples are listed below for transparency.
- Only 21 records (2.3%) are flagged by Layer 1 but not Layer 2 (Layer 1 surface-pattern false positives are rare).
- The divergence is **systematic, not random**: Layer 1 is under-sensitive for Family A. The κ = 0.245 reflects this structural under-detection, not noise.

**Five Layer-1-no, Layer-2-yes examples** (for §8.4 documentation):
[To be filled with concrete examples after manual inspection; this subsection is exploratory annotation, not part of the confirmatory analysis.]

**Implications**:
- The Layer 2 confirmatory delta_elicit = 0.237 is approximately half of the Layer 1 estimate (0.483). The Layer 1 estimate is therefore an **upper bound** for the magnitude of frame elicitation effect; the Layer 2 estimate is the more conservative, semantically valid estimate.
- Despite the smaller magnitude, Layer 2 still clearly exceeds the §6.4 threshold (0.237 / 0.150 = 1.58×), so the pre-registered GO decision is robust to Layer 2 conservativism.
- The substantial L1-no / L2-yes count is methodologically important and will be reported as a primary methodological finding in the manuscript: **regex-only evaluation systematically underestimates qualified-response markers in instruction-tuned LLMs**.

**Per-bucket breakdown** (Layer 1 only, for §13.10 internal record):

| Bucket | C0 | C1a-iii | C1b | delta_elicit (Layer 1) |
|---|---:|---:|---:|---:|
| health | 0.467 | 0.187 | 0.120 | 0.347 |
| legal | 0.560 | 0.213 | 0.080 | 0.480 |
| finance | 0.667 | 0.467 | 0.080 | 0.587 |
| interpersonal | 0.533 | 0.107 | 0.013 | 0.520 |

**Per-seed stability** (Family A only, Layer 1; n=100 per cell):

| seed | C0 | C1a-iii | C1b |
|---|---:|---:|---:|
| 42 | 0.570 | 0.220 | 0.080 |
| 7 | 0.560 | 0.220 | 0.080 |
| 123 | 0.540 | 0.290 | 0.060 |

Variance across seeds is ±0.02 for C0 and ±0.01 for C1b, demonstrating high stability under temperature=0.7 within the pre-registered seed set.

**Output file integrity**:

| File | Role | SHA256 |
|---|---|---|
| `stage1_results_20260518_100316.json` | Stage 1 main results (Layer 1 included) | `7f726e41361d650ab3a795d9d6c4e2b5fc101788ba5b0d09a0850ba99f1d966c` |
| `stage1_log_20260518_100316.log` | Modal run log | `dd56799312e292a5948c5e37a5e5eb96b7cd4e0e9353d749412fa3f8049ad5d9` |
| `judge_results_20260518_100316.json` | Layer 2 LLM-judge verdicts | `0273ac331d02f35f875d2a8f31826d8e9ffea4b1ce6f735be269fd477b07972d` |

These files are SHA-fixed at this v1.4 commit. Re-analysis with the same files reproduces all numerical results in §13.10.
...
| **Judge model retired by Anthropic** | **Substitute to active model in same family; report substitution + rationale in §13.x; preserve all other §1 settings; report cross-layer agreement so reviewers can assess substitution impact (occurred 2026-05-18, see §13.9)** |

---

**End of pre-registration v1.4**

Submitted by:
みなもと みなも(Minamo Minamoto)
2026-05-18

- v1.1 frozen at OSF registration: 2026-05-18 17:53 JST
- v1.2 post-registration update (DOI embed + §13.8 driver implementation): 2026-05-18 evening JST
- v1.3 Step C complete (pilot results filled + C1a-iii adopted per §5.3): 2026-05-18 18:46 JST
- v1.4 Steps D+E complete (Stage 1 main run + Layer 2 LLM-judge + §6.4 GO verdict + §13.9 model substitution deviation): 2026-05-18 20:40 JST
